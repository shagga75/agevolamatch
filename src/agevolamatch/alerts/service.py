...
from agevolamatch.alerts.formatting import format_alert_message, format_tender_alert_message
...
from agevolamatch.tenders.engine import match_tender_profile
...
def run_tender_alerts(
    ...
    """Tender counterpart of run_alerts - same dedup mechanism (SentAlert is
    keyed on source/source_id/content_hash, which both Incentive and Tender
    have via the shared Opportunity base), same AlertEvent/AlertRunSummary
    types; only the matching call and message formatting differ, per the
    project's "gare is a separate module" convention."""
    summary = AlertRunSummary(dry_run=dry_run)

    for subscription in subscriptions:
        profile = load_company_profile(subscription.profile)
        weights = (
            TenderScoringWeights.from_yaml(subscription.weights) if subscription.weights else DEFAULT_TENDER_WEIGHTS
        )
        results = match_tender_profile(tenders, profile, weights=weights)

        for result in results:
            if result.score < subscription.min_score:
                continue
            tender = result.tender
            if already_sent(session, subscription.name, tender.source, tender.source_id, tender.content_hash):
                continue

            if not dry_run:
                subject, body = format_tender_alert_message(subscription.name, result)
                for channel_name in subscription.channels:
                    channel = channels.get(channel_name)
                    if channel is None:
                        raise ValueError(f"Unknown alert channel {channel_name!r} in subscription {subscription.name!r}")
